...
int main() {
    ...
    // Define and add the wheel
    float wheel_IYY = wheel_mass * wheel_rad * wheel_rad / 2;
    float wheel_IXX = (wheel_mass / 12) * (3 * wheel_rad * wheel_rad + wheel_width * wheel_width);
    auto wheel = DEMSim.AddWavefrontMeshObject(GetDEMEDataFile("mesh/rover_wheels/viper_wheel_right.obj"), mat_type_wheel);
    ...
    // Define prescribed motions for different families
    // Family 1: Initial settling phase
    ...
    // Family 2: Real simulation phase with slip ratio 0.5
    DEMSim.SetFamilyPrescribedAngVel(2, "0", to_string_with_precision(w_r), "0", false);
    // Note: this wheel is not `dictated' by our prescription of motion (hence the false argument), because it
    ...
    // Define terrain particle properties
    ...
    // Create trackers and inspectors (before initialization)
    ...
    // Phase 1: Initial settling - put wheel in place and let it sink in
    ...
    // Phase 2: Switch wheel from free fall into DP test
    ...
    std::cout << "WheelDPSimplified demo exiting..." << std::endl;
    return 0;
}